...
monthly_hourly_file = Path('hourly_index_monthly_picks.csv')
hourly_files = sorted(Path('.').glob('hourly_index_picks_*.csv'))

if monthly_hourly_file.exists():
    hourly_file = monthly_hourly_file
elif hourly_files:
    hourly_file = hourly_files[-1]
else:
    raise FileNotFoundError("No hourly_index_monthly_picks.csv or hourly_index_picks_*.csv files found")
